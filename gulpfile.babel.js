 ...
 * Added Polling as the dockervolume is not triggering as the OS is not reporting changes, this is a workaround unless we change to use gulp-watch
 */
gulp.task(
  "default",
  gulp.parallel("styles", "customJS", () => {
    gulp.watch(config.watchStyles, {interval: 1000, usePolling: true}, gulp.parallel("styles")); // Reload on SCSS file changes.
    gulp.watch(config.watchJsCustom, {interval: 1000, usePolling: true}, gulp.series("customJS")); // Reload on customJS file changes.
    gulp.watch(config.imgSRC, {interval: 1000, usePolling: true}, gulp.series("images")); // Reload on customJS file changes.
  })
);
